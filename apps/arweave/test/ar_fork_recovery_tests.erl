...
test_orphaned_txs_are_remined_after_fork_recovery() ->
	...
	ar_test_node:start_peer(peer1, B0),
	ar_test_node:disconnect_from(peer1),
	TX = #tx{ id = TXID } = ar_test_node:sign_tx(Key, #{ denomination => 1, reward => ?BIG(1) }),
...
test_fork_recovery(Split) ->
	...
	{TX1, Chunks1} = ar_test_data_sync:tx(Wallet, {Split, 3}, v2, ?BIG(10)),
	...
	{PeerTX3, PeerChunks3} = ar_test_data_sync:tx(Wallet, {Split, 2}, v2, ?BIG(10)),
	...
	?debugFmt("Mined block ~s, height ~B.~n", [ar_util:encode(PeerB2#block.indep_hash),
			PeerB2#block.height]),
	{MainTX2, MainChunks2} = ar_test_data_sync:tx(Wallet, {Split, 1}, v2, ?BIG(10)),
	...
	{PeerTX4, PeerChunks4} = ar_test_data_sync:tx(Wallet, {Split, 2}, v2, ?BIG(10)),
	...
	{MainTX3, MainChunks3} = ar_test_data_sync:tx(Wallet, {Split, 1}, v2, ?BIG(10)),
