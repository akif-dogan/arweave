...
is_ready(StoreID) ->
    case catch gen_server:call(name(StoreID), is_ready, infinity) of
        {'EXIT', {Reason, {gen_server, call, _}}} ->
            ?LOG_WARNING([{event, is_ready_error},
                          {module, ?MODULE},
                          {name, name(StoreID)},
                          {store_id, StoreID},
                          {reason, Reason}]),
...
do_store_entropy(Entropies,
    ...
    case take_and_combine_entropy_slices(Entropies) of
        ...
        {ChunkEntropy, Rest} ->
            ...
            do_store_entropy(Rest, BucketEndOffset2, RangeStart, RangeEnd, Keys, RewardAddr, State)
    end.

record_chunk(PaddedEndOffset,
             Chunk,
             StoreID,
             StoreIDLabel,
             PackingLabel,
    ...
    %% End sanity checks
    StartOffset = big_chunk_storage:get_chunk_bucket_start(PaddedEndOffset),
    ...
    ReadEntropy =
        case CheckIsEntropyRecorded of
            ...
            true ->
                big_chunk_storage:get(StartOffset, StartOffset, StoreID)
        end,
    RecordChunk =
        case ReadEntropy of
            {error, _} = Error2 ->
...
record_entropy(ChunkEntropy, BucketEndOffset, StoreID, RewardAddr) ->
    ...
    {ChunkFileStart, Filepath, _Position, _ChunkOffset} =
        big_chunk_storage:locate_chunk_on_disk(EndOffset, StoreID),
    ...
    Chunk =
        case IsUnpackedChunkRecorded of
            true ->
                ...
                case big_chunk_storage:get(Byte, StartOffset, StoreID) of
                    not_found ->
                        {error, not_found};
                    {error, _} = Error ->
                        Error;
    ...
    Result =
        case Chunk of
            {error, _} = Error2 ->
                Error2;
            _ ->
                WriteChunkResult = big_chunk_storage:write_chunk(EndOffset, Chunk, #{}, StoreID),
                case WriteChunkResult of
                    {ok, Filepath} ->
                        ets:insert(chunk_storage_file_index, {{ChunkFileStart, StoreID}, Filepath}),
                        update_sync_records(IsUnpackedChunkRecorded,
                                            EndOffset,
                                            StoreID,
                                            RewardAddr);
                    Error2 ->
...
test_replica_2_9() ->
    ...
    try
        ...
        ?assertEqual({error, already_stored},
                     big_chunk_storage:put(?DATA_CHUNK_SIZE, C1, Packing, StoreID1)),
        ?assertEqual({error, already_stored},
                     big_chunk_storage:put(2 * ?DATA_CHUNK_SIZE, C1, Packing, StoreID1)),
        ?assertEqual({error, already_stored},
                     big_chunk_storage:put(3 * ?DATA_CHUNK_SIZE, C1, Packing, StoreID1)),
...
assert_get(Expected, Offset, StoreID) ->
    ...
    ?assertEqual(ExpectedResult,
                 big_chunk_storage:get(Offset - ?DATA_CHUNK_SIZE + 1, StoreID)),
    ?assertEqual(ExpectedResult,
                 big_chunk_storage:get(Offset - ?DATA_CHUNK_SIZE + 2, StoreID)),
    ?assertEqual(ExpectedResult,
