...
is_tx_blacklisted(TXID) ->
	ets:member(big_tx_blacklist, TXID).
...
initialize_state() ->
	...
	Dir = filename:join(DataDir, "big_tx_blacklist"),
		...
		sets:fold(
			fun	(TXID, Acc) when is_binary(TXID) ->
					case not sets:is_element(TXID, Whitelist)
							andalso not ets:member(big_tx_blacklist, TXID) of
						true ->
							[TXID | Acc];
						false ->
							Acc
				...
				({End, Start}, Acc) ->
					case ets:member(big_tx_blacklist, {End, Start}) of
						true ->
							Acc;
						false ->
							[{End, Start} | Acc]
