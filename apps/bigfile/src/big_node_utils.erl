...
test_block_validation() ->
	...
	PrevTX = big_test_node:sign_tx(main, Wallet, #{ reward => ?BIG(10),
			data => crypto:strong_rand_bytes(10 * 1024 * 1024) }),
	big_test_node:assert_post_tx_to_peer(main, PrevTX),
	big_test_node:mine(),
	[_ | _] = big_test_node:wait_until_height(main, 1),
	big_test_node:mine(),
	[{PrevH, _, _} | _ ] = big_test_node:wait_until_height(main, 2),
	PrevB = big_node:get_block_shadow_from_cache(PrevH),
	BI = big_node:get_block_index(),
	PartitionUpperBound = big_node:get_partition_upper_bound(BI),
	BlockAnchors = big_node:get_block_anchors(),
	RecentTXMap = big_node:get_recent_txs_map(),
	TX = big_test_node:sign_tx(main, Wallet, #{ reward => ?BIG(10),
			data => crypto:strong_rand_bytes(7 * 1024 * 1024), last_tx => PrevH }),
	big_test_node:assert_post_tx_to_peer(main, TX),
	big_test_node:mine(),
	[{H, _, _} | _] = big_test_node:wait_until_height(main, 3),
	B = big_node:get_block_shadow_from_cache(H),
	...
	?assertEqual({invalid, invalid_txs},
			validate(B#block{ txs = [TX#tx{ reward = ?BIG(201) }] }, PrevB, Wallets,
					BlockAnchors, RecentTXMap, PartitionUpperBound)),
	...
	RecentTXMap2 = big_node:get_recent_txs_map(),
	big_test_node:mine(),
	[{H2, _, _} | _ ] = big_test_node:wait_until_height(main, 4),
	B2 = big_node:get_block_shadow_from_cache(H2),
