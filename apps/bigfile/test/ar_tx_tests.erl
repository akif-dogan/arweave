...
-import(big_test_node, [wait_until_height/2, assert_wait_until_height/2,
...
rejects_txs_with_outdated_anchors_test_() ->
	{timeout, ?DEFAULT_EUNIT_TEST_TIMEOUT, fun() ->
		...
		TX1 = big_test_node:sign_v1_tx(Key, #{ last_tx => B0#block.indep_hash }),
		{ok, {{<<"400">>, _}, _, <<"Invalid anchor (last_tx).">>, _, _}} =
			big_test_node:post_tx_to_peer(peer1, TX1)
	end}.
...
accepts_gossips_and_mines(B0, TXFuns) ->
	...
	?assertEqual(
		lists:sort(TXIDs),
		lists:sort((big_test_node:remote_call(peer1, big_test_node, read_block_when_stored, [hd(PeerBI)]))#block.txs)
	),
	lists:foreach(
		fun(TX) ->
			?assertEqual(TX, big_test_node:remote_call(peer1, big_storage, read_tx, [TX#tx.id]))
		end,
		TXs
	),
...
polls_for_transactions_and_gossips_and_mines(B0, TXFuns) ->
	...
	try
		...
		_ = big_test_node:start(#{ b0 => B0, config => MainConfig2 }),
		PeerConfig2 = PeerConfig#config{ max_propagation_peers = 0 },
		_ = big_test_node:start_peer(peer1, #{ b0 => B0, config => PeerConfig2 }),
		%% Sign here after the node has started to get the correct price
		%% estimation from it.
		TXs = lists:map(fun(TXFun) -> TXFun() end, TXFuns),
		big_test_node:connect_to_peer(peer1),
	...
	after
		...
		big_test_node:set_config(peer1, PeerConfig)
	end.
...
keeps_txs_after_new_block(B0, FirstTXSetFuns, SecondTXSetFuns) ->
	...
	try
		...
		lists:foreach(
			fun(TX) ->
				big_test_node:post_tx_to_peer(main, TX)
			end,
			SecondTXSet ++ FirstTXSet
		),
		?assertEqual([], big_test_node:remote_call(peer1, big_mempool, get_all_txids, [])),
		...
		big_test_node:assert_wait_until_receives_txs(FirstTXSet -- SecondTXSet),
...
returns_error_when_txs_exceed_balance(BuildTXSetFun) ->
	...
	lists:foreach(
		fun(TX) ->
		big_test_node:assert_post_tx_to_peer(peer1, TX, false)
		end,
		TXs
	),

	big_test_node:assert_wait_until_receives_txs(BelowBalanceTXs),
	...
	big_test_node:remote_call(peer1, ets, delete, [ignored_ids, ExceedBalanceTX#tx.id]),
	{ok, {{<<"400">>, _}, _, _Body, _, _}} =
		big_http:req(#{
			method => post,
			peer => big_test_node:peer_ip(peer1),
			path => "/tx",
			body => big_serialize:jsonify(big_serialize:tx_to_json_struct(ExceedBalanceTX))
		}),
	?assertEqual({ok, ["overspend"]}, big_test_node:remote_call(peer1, big_tx_db, get_error_codes,
...
test_rejects_transactions_above_the_size_limit() ->
	...
	%% Start the node.
	_ = big_test_node:start_peer(peer1, B0),
	_ = big_test_node:connect_to_peer(peer1),
	SmallData = random_v1_data(?TX_DATA_SIZE_LIMIT),
	BigData = random_v1_data(?TX_DATA_SIZE_LIMIT + 1),
	GoodTX = big_test_node:sign_v1_tx(Key1, #{ data => SmallData }),
	big_test_node:assert_post_tx_to_peer(peer1, GoodTX),
	BadTX = big_test_node:sign_v1_tx(Key2, #{ data => BigData }),
	?assertMatch(
		{ok, {{<<"400">>, _}, _, <<"Transaction verification failed.">>, _, _}},
		big_test_node:post_tx_to_peer(peer1, BadTX)
	),
	?assertMatch(
		{ok, ["tx_fields_too_large"]},
		big_test_node:remote_call(peer1, big_tx_db, get_error_codes, [BadTX#tx.id])
	).
...
test_accepts_at_most_one_wallet_list_anchored_tx_per_block() ->
	...
	_ = big_test_node:start_peer(peer1, B0),
	_ = big_test_node:connect_to_peer(peer1),
	TX1 = big_test_node:sign_v1_tx(Key),
	big_test_node:assert_post_tx_to_peer(peer1, TX1),
	big_test_node:mine(peer1),
	assert_wait_until_height(peer1, 1),
	TX2 = big_test_node:sign_v1_tx(Key, #{ last_tx => TX1#tx.id }),
	big_test_node:assert_post_tx_to_peer(peer1, TX2),
	TX3 = big_test_node:sign_v1_tx(Key, #{ last_tx => TX2#tx.id }),
	{ok, {{<<"400">>, _}, _, <<"Invalid anchor (last_tx from mempool).">>, _, _}} = big_test_node:post_tx_to_peer(peer1, TX3),
	TX4 = big_test_node:sign_v1_tx(Key, #{ last_tx => B0#block.indep_hash }),
	big_test_node:assert_post_tx_to_peer(peer1, TX4),
	big_test_node:mine(peer1),
	PeerBI = assert_wait_until_height(peer1, 2),
	B2 = big_test_node:remote_call(peer1, big_test_node, read_block_when_stored, [hd(PeerBI)]),
...
test_does_not_allow_to_spend_mempool_tokens() ->
	...
	_ = big_test_node:start_peer(peer1, B0),
	_ = big_test_node:connect_to_peer(peer1),
	TX1 = big_test_node:sign_v1_tx(Key1, #{ target => big_wallet:to_address(Pub2), reward => ?BIG(1),
			quantity => ?BIG(2) }),
	big_test_node:assert_post_tx_to_peer(peer1, TX1),
	TX2 = big_test_node:sign_v1_tx(
		Key2,
		#{
			target => big_wallet:to_address(Pub1),
			reward => ?BIG(1),
			quantity => ?BIG(1),
	...
	{ok, {{<<"400">>, _}, _, _, _, _}} = big_test_node:post_tx_to_peer(peer1, TX2),
	?assertEqual({ok, ["overspend"]}, big_test_node:remote_call(peer1, big_tx_db, get_error_codes, [TX2#tx.id])),
	big_test_node:mine(peer1),
	PeerBI = assert_wait_until_height(peer1, 1),
	B1 = big_test_node:remote_call(peer1, big_test_node, read_block_when_stored, [hd(PeerBI)]),
	?assertEqual([TX1#tx.id], B1#block.txs),
	TX3 = big_test_node:sign_v1_tx(
		Key2,
		#{
			target => big_wallet:to_address(Pub1),
			reward => ?BIG(1),
	...
	big_test_node:assert_post_tx_to_peer(peer1, TX3),
	big_test_node:mine(peer1),
	PeerBI2 = assert_wait_until_height(peer1, 2),
	B2 = big_test_node:remote_call(peer1, big_test_node, read_block_when_stored, [hd(PeerBI2)]),
...
test_does_not_allow_to_replay_empty_wallet_txs() ->
	...
	_ = big_test_node:start_peer(peer1, B0),
	TX1 = big_test_node:sign_v1_tx(Key1, #{ target => big_wallet:to_address(Pub2), reward => ?BIG(6),
	...
	TX2 = big_test_node:sign_v1_tx(Key2, #{ target => big_wallet:to_address(Pub1), reward => Balance - ?BIG(1),
	...
	TX3 = big_test_node:sign_v1_tx(Key1, #{ target => big_wallet:to_address(Pub2), reward => ?BIG(6),
	...
	big_test_node:remote_call(peer1, ets, delete, [ignored_ids, TX2#tx.id]),
	{ok, {{<<"400">>, _}, _, <<"Invalid anchor (last_tx).">>, _, _}} =
		big_test_node:post_tx_to_peer(peer1, TX2).
	...
	lists:foldl(
		fun(Group, Height) ->
			...
			?assertEqual(
				...
				lists:sort(
					(big_test_node:remote_call(peer1, big_test_node, read_block_when_stored, [hd(PeerBI)]))#block.txs
				),
...
test_drops_v1_txs_exceeding_mempool_limit() ->
	...
	TXs = lists:map(
		fun(N) ->
			big_test_node:sign_v1_tx(Key, #{ last_tx => B0#block.indep_hash,
	...
	{ok, Mempool1} = big_http_iface_client:get_mempool(big_test_node:peer_ip(peer1)),
	...
	Last = lists:last(TXs),
	{ok, {{<<"200">>, _}, _, <<"OK">>, _, _}} = big_test_node:post_tx_to_peer(peer1, Last, false),
	{ok, Mempool2} = big_http_iface_client:get_mempool(big_test_node:peer_ip(peer1)),
...
drops_v2_txs_exceeding_mempool_limit() ->
	...
	TXs = lists:map(
		fun(N) ->
			big_test_node:sign_tx(Key, #{ last_tx => B0#block.indep_hash,
	...
	StrippedTX = big_test_node:sign_tx(Key, #{ last_tx => B0#block.indep_hash,
			data => BigChunk, tags => [{<<"nonce">>, integer_to_binary(12)}] }),
	big_test_node:assert_post_tx_to_peer(peer1, StrippedTX#tx{ data = <<>> }),
	{ok, Mempool3} = big_http_iface_client:get_mempool(big_test_node:peer_ip(peer1)),
...
joins_network_successfully() ->
	...
	{TXs, _} = lists:foldl(
		fun(Height, {TXs, LastTX}) ->
			{TX, AnchorType} = case rand:uniform(4) of
				1 ->
					{big_test_node:sign_v1_tx(Key, #{ last_tx => LastTX, reward => ?BIG(10000) }), tx_anchor};
				2 ->
					{big_test_node:sign_v1_tx(Key, #{ last_tx => big_test_node:get_tx_anchor(peer1), reward => ?BIG(10000),
							tags => [{<<"nonce">>, integer_to_binary(rand:uniform(100))}] }),
							block_anchor};
				3 ->
					{big_test_node:sign_tx(Key, #{ last_tx => LastTX, target => Addr,
							reward => ?BIG(10000) }), tx_anchor};
				4 ->
					{big_test_node:sign_tx(Key, #{ last_tx => big_test_node:get_tx_anchor(peer1), reward => ?BIG(10000),
							tags => [{<<"nonce">>, integer_to_binary(rand:uniform(100))}]}),
							block_anchor}
			end,
			big_test_node:assert_post_tx_to_peer(peer1, TX),
	...
	BI = big_test_node:remote_call(peer1, big_node, get_block_index, []),
	?assertEqual(ok, big_test_node:wait_until_block_index(BI)),
	TX1 = big_test_node:sign_tx(Key, #{ last_tx => element(1, lists:nth(?MAX_TX_ANCHOR_DEPTH + 1, BI)) }),
	{ok, {{<<"400">>, _}, _, <<"Invalid anchor (last_tx).">>, _, _}} =
		big_test_node:post_tx_to_peer(main, TX1),
	...
	lists:foreach(
		fun({TX, AnchorType}) ->
			Reply = big_test_node:post_tx_to_peer(main, TX),
			case AnchorType of
				tx_anchor ->
	...
	TX2 = big_test_node:sign_tx(main, Key, #{ last_tx => element(1, lists:nth(?MAX_TX_ANCHOR_DEPTH, BI)) }),
	...
	TX3 = big_test_node:sign_tx(peer1, Key, #{ last_tx => element(1, lists:nth(?MAX_TX_ANCHOR_DEPTH - 1, BI)) }),
	big_test_node:assert_post_tx_to_peer(peer1, TX3),
	big_test_node:mine(peer1),
	...
	TX4 = big_test_node:sign_tx(peer1, Key, #{ last_tx => element(1, lists:nth(?MAX_TX_ANCHOR_DEPTH, BI2)) }),
	big_test_node:assert_post_tx_to_peer(peer1, TX4),
	big_test_node:assert_wait_until_receives_txs([TX4]),
	big_test_node:mine(peer1),
...
recovers_from_forks(ForkHeight) ->
	...
	PreForkTXs = lists:foldl(
		fun(Height, TXs) ->
			TX = big_test_node:sign_v1_tx(Key, #{ last_tx => big_test_node:get_tx_anchor(peer1),
	...
	PostTXToMain =
		fun() ->
			UnsignedTX = #{ last_tx => big_test_node:get_tx_anchor(main),
					tags => [{<<"nonce">>, random_nonce()}], reward => ?BIG(1) },
			TX = case rand:uniform(2) of
				1 ->
					big_test_node:sign_tx(main, Key, UnsignedTX);
				2 ->
					big_test_node:sign_v1_tx(main, Key, UnsignedTX)
			end,
	...
	PostTXToPeer =
		fun() ->
			UnsignedTX = #{ last_tx => big_test_node:get_tx_anchor(peer1),
					tags => [{<<"nonce">>, random_nonce()}] },
			TX = case rand:uniform(2) of
				1 ->
					big_test_node:sign_tx(Key, UnsignedTX);
				2 ->
					big_test_node:sign_v1_tx(Key, UnsignedTX)
			end,
			big_test_node:assert_post_tx_to_peer(peer1, TX),
			[TX]
		end,
	...
	big_test_node:connect_to_peer(peer1),
	TX2 = big_test_node:sign_tx(Key, #{ last_tx => big_test_node:get_tx_anchor(peer1),
			tags => [{<<"nonce">>, random_nonce()}] }),
	big_test_node:assert_post_tx_to_peer(peer1, TX2),
	big_test_node:assert_wait_until_receives_txs([TX2]),
	big_test_node:mine(peer1),
...
one_wallet_list_one_block_anchored_txs(Key, B0) ->
	...
	TX1Fun = fun() ->
		case KeyType of
			?RSA_KEY_TYPE ->
				big_test_node:sign_v1_tx(Key, #{ reward => ?BIG(1) });
			?ECDSA_KEY_TYPE ->
				big_test_node:sign_tx(Key, #{ reward => ?BIG(1), last_tx => <<>> })
		end end,
	TX2Fun = fun() ->
		case KeyType of
			?RSA_KEY_TYPE ->
				big_test_node:sign_v1_tx(Key, #{ reward => ?BIG(1),
						last_tx => B0#block.indep_hash });
			?ECDSA_KEY_TYPE ->
				big_test_node:sign_tx(Key, #{ reward => ?BIG(1),
						last_tx => B0#block.indep_hash })
		end end,
	[TX1Fun, TX2Fun].
...
block_anchor_txs_spending_balance_plus_one_more(Key, B0) ->
	TX1 = big_test_node:sign_v1_tx(Key, #{ denomination => 1,
			reward => ?BIG(10), last_tx => B0#block.indep_hash }),
	TX2 = big_test_node:sign_v1_tx(Key, #{ denomination => 1,
			reward => ?BIG(10), last_tx => B0#block.indep_hash }),
	TX3 = big_test_node:sign_v1_tx(Key, #{ denomination => 1,
			reward => ?BIG(1), last_tx => B0#block.indep_hash }),
	[TX1, TX2, TX3].

mixed_anchor_txs_spending_balance_plus_one_more(Key, B0) ->
	TX1 = big_test_node:sign_v1_tx(Key, #{ denomination => 1, reward => ?BIG(10), last_tx => <<>> }),
	TX2 = big_test_node:sign_v1_tx(Key, #{ denomination => 1, reward => ?BIG(5),
			last_tx => B0#block.indep_hash }),
	TX3 = big_test_node:sign_v1_tx(Key, #{ denomination => 1, reward => ?BIG(2),
			last_tx => B0#block.indep_hash }),
	TX4 = big_test_node:sign_v1_tx(Key, #{ denomination => 1,
			reward => ?BIG(3), last_tx => B0#block.indep_hash }),
	TX5 = big_test_node:sign_v1_tx(Key, #{ denomination => 1,
			reward => ?BIG(1), last_tx => B0#block.indep_hash }),
	[TX1, TX2, TX3, TX4, TX5].
...
grouped_txs() ->
	...
	TX2 = big_test_node:sign_v1_tx(Key2, #{ reward => ?BIG(1), data => Chunk2,
			last_tx => B0#block.indep_hash }),
...
assert_block_txs(Node, TXs, BI) ->
	TXIDs = lists:map(fun(TX) -> TX#tx.id end, TXs),
	B = big_test_node:remote_call(Node, big_test_node, read_block_when_stored, [hd(BI)]),
