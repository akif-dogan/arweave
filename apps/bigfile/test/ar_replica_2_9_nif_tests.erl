...
setup_replica_2_9() ->
	FastState = big_mine_randomx:init_fast2(rxsquared, ?RANDOMX_PACKING_KEY, 0, 0,
			erlang:system_info(dirty_cpu_schedulers_online)),
	LightState = big_mine_randomx:init_light2(rxsquared, ?RANDOMX_PACKING_KEY, 0, 0),
...
test_state({FastState, LightState}) ->
	...
	?assertEqual(
		...
		big_mine_randomx:info(FastState)
	),
	?assertEqual(
		{ok, {rxsquared, light, 0, 2097152}},
		big_mine_randomx:info(LightState)
	),

	{ok, {_, _, _, ScratchpadSize}} = big_mine_randomx:info(FastState),
	?assertEqual(?RANDOMX_SCRATCHPAD_SIZE, ScratchpadSize).

test_vectors({FastState, _LightState}) ->
	Key = << 1 >>,
	Entropy = big_mine_randomx:randomx_generate_replica_2_9_entropy(FastState, Key),
	...
	Key2 = << 2 >>,
	Entropy2 = big_mine_randomx:randomx_generate_replica_2_9_entropy(FastState, Key2),
	...
	{ok, Packed} = big_mine_randomx:randomx_encrypt_replica_2_9_sub_chunk(
	...
	{ok, Unpacked} = big_mine_randomx:randomx_decrypt_replica_2_9_sub_chunk(
...
test_pack_unpack_sub_chunks({State, _LightState}) ->
	Key = << 0:256 >>,
	SubChunk = << 0:(8192 * 8) >>,
	Entropy = big_mine_randomx:randomx_generate_replica_2_9_entropy(State, Key),
...
unpack_sub_chunks([PackedSubChunk | PackedSubChunks], Index, SubChunk, Entropy) ->
	{ok, UnpackedSubChunk} = big_mine_randomx:randomx_decrypt_replica_2_9_sub_chunk2(
